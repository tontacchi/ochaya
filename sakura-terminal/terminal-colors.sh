...
color_fg="ff/d5/cd" # ffd5cd Sakura
color_bg="25/1f/2c" # 251f2c Dark Plum
...
if [ -n "$ITERM_SESSION_ID" ]; then
	...
	put_template_var 10 $color_fg
	if [ "$BASE16_SHELL_SET_BACKGROUND" != false ]; then
		put_template_var 11 $color_bg
		if [ "${TERM%%-*}" = "rxvt" ]; then
			put_template_var 708 $color_bg # internal border (rxvt)
		fi
	fi
